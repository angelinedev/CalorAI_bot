...
const STORAGE_KEY = 'calorai-mobile-session';
const DEFAULT_API_BASE_URL = 'https://your-ngrok-or-public-url';
...
async function configureNotifications() {
  if (Platform.OS === 'android') {
    await Notifications.setNotificationChannelAsync('calor-reminders', {
      name: 'CalorAI Reminders',
...
async function scheduleDailyNotifications(summary) {
  await Notifications.cancelAllScheduledNotificationsAsync();

  await Notifications.scheduleNotificationAsync({
    content: {
      title: 'CalorAI reminder',
      body: 'Log dinner and keep your streak going.',
      sound: true
    },
    trigger: {
      hour: 20,
  ...
  await Notifications.scheduleNotificationAsync({
    content: {
      ...
      body: `${summary.meals.length} meals logged today - ${formatNumber(summary.totals.calories)} kcal`,
      sound: true
    },
    trigger: {
      hour: 21,
...
export default function App() {
  ...
  useEffect(() => {
    if (!sessionToken || !apiBaseUrl || booting) {
      return undefined;
    }

    let cancelled = false;

    async function syncNow(showLoader = false) {
      try {
        if (showLoader) {
          setSyncing(true);
        }

        const nextDashboard = await apiRequest(apiBaseUrl, sessionToken, '/api/me/dashboard');
        if (cancelled) {
          return;
        }
        ...
        await scheduleDailyNotifications(nextDashboard.summary);
    ...
    syncNow(true);
    const interval = setInterval(() => syncNow(false), POLL_INTERVAL_MS);
  ...
  }, [apiBaseUrl, booting, sessionToken, username]);

  async function handleLogin() {
    if (!apiBaseUrl || !username || !password) {
      setMessage('Enter API URL, username, and password.');
      return;
    }
    ...
    try {
      setLoading(true);
      setMessage('');
      const payload = await apiRequest(apiBaseUrl, null, '/api/auth/login', {
        method: 'POST',
  ...
  async function handleLogout() {
    try {
      if (sessionToken) {
        await apiRequest(apiBaseUrl, sessionToken, '/api/auth/logout', {
          method: 'POST',
          body: '{}'
        });
      }
  ...
  async function refreshAnalysis() {
    try {
      setLoading(true);
      const payload = await apiRequest(apiBaseUrl, sessionToken, '/api/me/analysis');
      setAnalysis(payload);
    } catch (error) {
      setMessage(error.message);
    } finally {
  ...
  async function handleMealSubmit() {
    if (!draft.name || !draft.calories) {
      setMessage('Meal name and calories are required.');
      return;
    }
    ...
    try {
      setLoading(true);
      setMessage('');
      const path = editingMealId ? `/api/me/meals/${editingMealId}` : '/api/me/meals';
      await apiRequest(apiBaseUrl, sessionToken, path, {
        method: editingMealId ? 'PATCH' : 'POST',
        body: JSON.stringify(draft)
      });
      setDraft(emptyDraft);
      setEditingMealId(null);
      await refreshDashboardManual();
    } catch (error) {
      setMessage(error.message);
    } finally {
      setLoading(false);
  ...
  async function handleDeleteMeal(mealId) {
    try {
      await apiRequest(apiBaseUrl, sessionToken, `/api/me/meals/${mealId}`, {
        method: 'DELETE',
        body: '{}'
      });
    ...
    return (
      <SafeAreaView style={styles.screen}>
        ...
        <View style={styles.centered}>
          <Text style={styles.title}>CalorAI Mobile</Text>
          <Text style={styles.muted}>Preparing your mobile workspace...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.headerCard}>
          <Text style={styles.eyebrow}>Expo Go Companion</Text>
          <Text style={styles.title}>CalorAI Mobile</Text>
          <Text style={styles.subtitle}>
            Shared backend, near-real-time meal sync, and daily reminder notifications.
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TextInput
            ...
            autoCapitalize="none"
            placeholder="https://your-ngrok-or-public-url"
            style={styles.input}
          />
          <TextInput
            value={username}
          ...
          <>
            <View style={styles.summaryRow}>
              <View style={styles.metricCard}>
                ...
                <Text style={styles.metricValue}>{formatNumber(dashboard.summary.totals.calories)}</Text>
              </View>
              <View style={styles.metricCard}>
                <Text style={styles.metricLabel}>Meals</Text>
                <Text style={styles.metricValue}>{dashboard.summary.meals.length}</Text>
              </View>
              <View style={styles.metricCard}>
                <Text style={styles.metricLabel}>Protein</Text>
                <Text style={styles.metricValue}>{formatNumber(dashboard.summary.totals.protein)}g</Text>
              </View>
            </View>
            ...
            <View style={styles.card}>
              <View style={styles.rowSpread}>
                <View style={styles.flexBody}>
                  <Text style={styles.sectionTitle}>{user?.displayName || user?.username}</Text>
                  <Text style={styles.muted}>Last synced: {formatDateTime(lastSyncedAt)}</Text>
                </View>
                <Pressable style={styles.secondaryButton} onPress={refreshDashboardManual}>
                  <Text style={styles.secondaryButtonText}>{syncing ? 'Syncing...' : 'Sync now'}</Text>
                </Pressable>
              </View>
              <Text style={styles.helper}>Telegram logs appear here automatically without manual refresh.</Text>
            </View>

            <View style={styles.card}>
              <View style={styles.rowSpread}>
                <Text style={styles.sectionTitle}>Daily analysis</Text>
                <Pressable style={styles.secondaryButton} onPress={refreshAnalysis}>
                  <Text style={styles.secondaryButtonText}>{loading ? 'Working...' : 'Refresh'}</Text>
                </Pressable>
              </View>
              {analysis ? (
                <>
                  <Text style={styles.analysisHeadline}>{analysis.analysis.headline}</Text>
                  <Text style={styles.helper}>{analysis.analysis.advice}</Text>
                </>
              ) : (
                <Text style={styles.helper}>Pull your latest daily analysis on demand.</Text>
              )}
            </View>

            <View style={styles.card}>
              <Text style={styles.sectionTitle}>{editingMealId ? 'Edit meal' : 'Log a meal'}</Text>
              <TextInput
                value={draft.name}
              ...
              <View style={styles.grid}>
                ...
                <TextInput
                  value={draft.protein}
                  onChangeText={(value) => setDraft((current) => ({ ...current, protein: value }))}
                  keyboardType="numeric"
                  placeholder="Protein"
                  style={[styles.input, styles.gridInput]}
                />
                <TextInput
                  value={draft.carbs}
                  onChangeText={(value) => setDraft((current) => ({ ...current, carbs: value }))}
                  keyboardType="numeric"
                  placeholder="Carbs"
                  style={[styles.input, styles.gridInput]}
                />
                <TextInput
                  value={draft.fats}
                  onChangeText={(value) => setDraft((current) => ({ ...current, fats: value }))}
                  keyboardType="numeric"
                  placeholder="Fats"
                  style={[styles.input, styles.gridInput]}
                />
              ...
              <View style={styles.row}>
                <Pressable style={styles.primaryButton} onPress={handleMealSubmit} disabled={loading}>
                  <Text style={styles.primaryButtonText}>{editingMealId ? 'Save meal' : 'Add meal'}</Text>
                </Pressable>
                {editingMealId ? (
                  <Pressable
                  ...
                  <View style={styles.rowSpread}>
                    <View style={styles.mealBody}>
                      ...
                      <Text style={styles.muted}>
                        {formatNumber(meal.calories)} kcal - P{formatNumber(meal.protein)} / C
                        {formatNumber(meal.carbs)} / F{formatNumber(meal.fats)}
                      </Text>
                      <Text style={styles.muted}>{formatDateTime(meal.loggedAt)}</Text>
                    </View>
                    <View style={styles.actionColumn}>
                      <Pressable style={styles.secondaryButton} onPress={() => startEditing(meal)}>
                        <Text style={styles.secondaryButtonText}>Edit</Text>
                      </Pressable>
                      <Pressable style={styles.dangerButton} onPress={() => handleDeleteMeal(meal.id)}>
            ...
            <View style={styles.card}>
              ...
              {(dashboard.trend || []).map((item) => (
                <View key={item.day} style={styles.trendRow}>
                  <View>
                    <Text style={styles.mealTitle}>{item.day}</Text>
                    <Text style={styles.muted}>{item.mealCount} meals</Text>
                  </View>
                  <Text style={styles.metricMini}>{formatNumber(item.calories)} kcal</Text>
                </View>
              ))}
            </View>
          </>
